    ...
    <div :class="marginTop">
      <div class="col-m-1 profile-img-div">
        ...
        <picture>
          ...
          <img :src="getImgUrl(this.imgUrl)" class="img profile-img" alt=""/>
        </picture>
      </div>
      <div :class="witchSize">
        ...
        <div class="mizouniat-container">
          <img
            :src="mizounImgUrl(isSelectedMizoun)"
            alt
            class="mizoun-img"
            @click.once="mizounOnClick"
          />
          <h3 class="mizount-count">{{mizoun}}</h3>
          <img
            :src="naMizounImgUrl(isSelectedNamizoun)"
            alt
            class="namizoun-img"
            @click.once="namizounOnClick"
          />
...
export default {
  ...
  methods: {
    reply () {
      ...
      if (this.isBig) {
        this.$emit('isReply', this.replyData)
      } else {
        this.$emit('isReply', this.replyData)
      }
    },
    ...
    naMizounImgUrl (isClicked) {
      if (isClicked) {
        return require('../../assets//icons/selected-naMizoun.png')
      } else {
        return require('../../assets/icons/unSelected-namizoun.png')
      }
    },
    mizounOnClick () {
      this.mizoun += 1
      this.$store
        .dispatch('submitMizoun', {
          commentId: this.id,
          publisherId: this.publisherId
        })
        .then(() => {
          this.isSelectedMizoun = !this.isSelectedMizoun
          if (this.isSelectedNamizoun) {
            this.isSelectedNamizoun = false
          }
        })
    },
    namizounOnClick () {
      this.namizoun += 1
      this.$store
        .dispatch('submitNamizoun', {
          commentId: this.id,
          publisherId: this.publisherId
        })
        .then(() => {
          this.isSelectedNamizoun = !this.isSelectedNamizoun
          if (this.isSelectedMizoun) {
            this.isSelectedMizoun = false
          }
        })
    },
    getImgUrl (path) {
